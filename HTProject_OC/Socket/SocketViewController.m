...
* 延伸：为什么连接的时候是三次握手，关闭的时候却是四次握手？
* 当被动方收到主动方的FIN报文通知时，它仅仅表示主动方没有数据再发送给被动方了。但未必被动方所有的数据都完整的发送给了主动方，所以被动方不会马上关闭SOCKET,它可能还需要发送一些数据给主动方后，再发送FIN报文给主动方，告诉主动方同意关闭连接，所以这里四次挥手
*****************************************************************************************************/
 ...
 * 处理方式：
 * 一、给数据包的头尾加上标记（如：start123456789end）。
 * 二、在数据包头部加上内容的长度（如：PACKAGELENGTH:0009123456789）。
 ****************************************************************************************************/
