...
+ (SecKeyRef)loadPublicKeyWithPath:(NSString *)derFilePath {
    NSData *derData = [[NSData alloc] initWithContentsOfFile:derFilePath];
    if (derData.length > 0) {
        return [self loadPublicKeyWithData:derData];
    }
    NSLog(@"load public key fail with path: %@", derFilePath);
    return nil;
}

+ (SecKeyRef)loadPublicKeyWithData:(NSData *)derData {
...
+ (NSData *)rsaEncryptData:(NSData *)data {
    SecKeyRef key = [self loadPublicKeyWithPath:[[NSBundle mainBundle] pathForResource:@"public_key" ofType:@"der"]];
    //获取秘钥长度，默认生成的密钥长度为1024bit即128byte
...
+ (SecKeyRef)loadPrivateKeyWithPath:(NSString *)p12FilePath password:(NSString *)p12Password {
    NSData *data = [NSData dataWithContentsOfFile:p12FilePath];
    if (data.length > 0) {
        return [self loadPrivateKeyWithData:data password:p12Password];
    }
    NSLog(@"load private key fail with path: %@", p12FilePath);
    return nil;
}

+ (SecKeyRef)loadPrivateKeyWithData:(NSData *)p12Data password:(NSString *)p12Password {
...
+ (NSData *)rsaDecryptData:(NSData *)data {
    SecKeyRef key = [self loadPrivateKeyWithPath:[[NSBundle mainBundle] pathForResource:@"private_key" ofType:@"p12"] password:@"Gj123456"];
